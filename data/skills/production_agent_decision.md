...
- **审核规则**：仅阶段4（构建分镜表）需要审核，全部场次确认保存后才派发监督层；单场写入成功不等于阶段4完成
...
| 审核 | **需要** → 全部场次核对保存完成后自动派发监督层 |

**阶段特有约束：** `associateAssetsIds` 中的索引必须指向资产库中实际存在的资产。

**阶段4逐场调度与中断恢复（遵循执行层分镜表 Skill）：**

1. 首次派发时要求执行层按需读取当前剧本、导演规划和已有分镜表，确定真实场次数 `M`，判断短篇整表模式或长篇逐场模式；已有场次、任务标识及总场次数必须与原任务一致，不得凭历史记忆臆造已保存的场次。
2. 短篇可以按旧协议一次输出完整的无属性 `<storyboardTable>`；长篇采用逐场模式，**每次 `run_sub_agent_storyboard_table` 只能处理一场 `N`**。本集后续派发沿用首次确定的同一 `task` 和 `M`，并明确派发“仅生成第N场、逐场模式”；绝不可要求一次子 Agent 回复输出多个同名标签。
3. 一场输出结束后，必须让执行层**重新读取工作区已保存的分镜表**确认该场场头真实存在、内容完整，才能派发下一缺失场；子 Agent 的文字确认、XML 流式输出或内存摘要不代表已落库。未确认时停止推进，报告尚未核实保存的场次；不得把本场计入完成。
4. 在同一制作任务继续或断线恢复时，由执行层读取当前已保存场头 `## 场N：`，与 `1…M` 对照，按序仅补缺场；若工作区已存在旧版整表或发生任务标识、总场数、人工编辑冲突，停止自动覆盖并说明原因。不要自行清空或重写已保存内容。
5. 只有执行层核实本集 `1…M` 场均在工作区读回、场次和台词覆盖符合剧本后，阶段4才算完成；**此前不派发监督层、阶段5或后续阶段**。若模型本轮无法继续，报告实际已保存与缺失场次，保持现有进度供后续继续。
...
阶段4全部场次已核实保存后：
1. 将执行层返回的完成确认展示给用户
2. **紧接着自动调用监督层审核**（无需等待用户指示）；逐场模式单场完成时不得触发审核

```
run_sub_agent_supervision(
  prompt: "请审核【分镜表】的完整产出物。审核维度：{维度列表}"
)
```
...
| 明确指定阶段 | 检查前置条件 → 派发该阶段 |
| "从头开始" / "完整制作" | 从阶段1顺序执行 |
| "继续" / "下一步" | `deepRetrieve` 获取进度 → 从当前阶段继续；阶段4须让执行层重新读取已保存场次，仅补缺场 |
...
2. **审核结果展示**：阶段4全剧分镜表保存后由监督层审核，展示报告并等待用户反馈
...
| 场景 | 处理 |
|------|------|
| 执行层返回错误 | 分析原因，调整指令重新派发（最多重试2次）；阶段4重试前先读回已保存场次 |
