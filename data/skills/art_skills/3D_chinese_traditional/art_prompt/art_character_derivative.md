...
| 格式 | 直接输出可用的提示词正文，不使用代码块、标题、表格、解释或方案对比 |

### 完整服化叠加（四栏设定图）

以角色基础形象图为唯一身份参考，仅进行服化妆造叠加；保持底模的脸型、五官比例、年龄感、体型与自然站姿不变。国风3D，高精度建模与PBR材质，电影级柔和光影。根据用户线索选择{基础妆/轻妆/正式妆}并描述必要的眉眼、唇色与肤感；发型为{造型类型}，{发饰描述}；服饰为{主色+款式}，{材质与工艺}；配饰为{必要配饰}。只保留用户设定真正需要的服化元素，不添加手持道具、场景或动作。
同一画面从左到右四栏：头顶到锁骨的人像特写、正面全身、90°侧面全身、背面全身；四栏面容、妆容、发型、发饰、服饰与配饰完全一致。自然站立，素灰纯色背景 #B8B8B8，均匀柔光，无硬阴影。全身从头顶到脚底完整入画，不生成文字、水印或logo。
