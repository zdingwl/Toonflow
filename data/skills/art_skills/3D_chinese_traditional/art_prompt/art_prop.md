...
4. **尺度明确** — 通过主体占画面比例与常识尺度感表达真实尺寸，禁止依赖文字标注或尺寸标签
...
古风道具四宫格设定图，国风3D，高精度建模与PBR材质，电影级柔和光影。主体为{道具类型/名称}，{核心造型与功能特征}；材质为{材质描述}，{工艺/装饰描述}，{状态描述}，纹理与光泽符合真实材质。道具独立静物陈列，不被任何人物持有、佩戴或使用。
同一画面采用 2×2 四宫格：左上正面、右上侧面、左下背面、右下材质/工艺细节特写；四格保持同一件道具的造型、比例、材质与状态一致。素灰纯色背景 #B8B8B8，均匀柔光，无硬阴影，道具在每格中完整清晰。画面中不出现人物、手、手指、手臂或其他肢体，不生成文字、尺寸标注、水印或logo。
