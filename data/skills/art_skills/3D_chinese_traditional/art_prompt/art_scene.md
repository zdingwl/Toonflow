...
4. **3D为锚** — 一切画面以高质量3D渲染为标准，拒绝平面贴图、低质CG动画感与塑料质感；追求体积光、环境光遮蔽与自然景深等电影渲染效果
...
> 以连贯自然语言输出单画面场景提示词；镜头光学词只保留与当前场景真正相关的 1-2 项，禁止同义词堆叠。

古风场景概念主视图，国风3D，高精度建模与PBR材质，电影级光影。{室内/室外}，{场景类型}，{用户已明确的朝代/季节/时间信息}。空间从前景{元素}、中景{元素}到后景{元素}形成清晰纵深；{主色调与氛围}，{天气或空气状态}。重点表现{木/石/玉/布/水等关键材质}的真实纹理与自然使用痕迹，光线遵循明确物理方向，保留体积感、环境光遮蔽与自然空气透视。采用最能代表场景主体的单画面自然观察视角，构图稳定，不拼图、不分屏。画面中不出现人物、人影或人体轮廓，不生成文字、水印或logo。
