...
4. 对识别出的每条衍生按下方字段规则生成完整 `name`/`desc`；资产类型沿用父资产类型，由写入工具自动确定
5. 简单说明本次新增的衍生资产内容（200 字以内）
6. 若全部资产均无需衍生，返回"无需衍生资产"，流程结束
7. 对每条新增衍生资产**逐条调用** `add_deriveAsset` 写入（新增时 `id` 填 `null`，并完整填写 `assetsId`/`name`/`desc`；不要传入 `type`）
8. 全部调用成功后再返回简短确认（例如："已完成衍生资产写入，共 N 条"）；若工具返回失败，不得将该条计为成功
...
- 资产类型不属于 `add_deriveAsset` 入参：写入工具根据 `assetsId` 对应父资产的 `type` 自动继承。角色衍生沿用 `role`，场景衍生沿用 `scene`；本阶段不生成道具或片段级衍生。
