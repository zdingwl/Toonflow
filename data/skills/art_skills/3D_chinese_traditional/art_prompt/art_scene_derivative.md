...
基于参考场景图生成同一空间的衍生主视图，保持建筑结构、空间布局、核心材质、风格与光源逻辑一致。国风3D，高精度建模与PBR材质，电影级光影。仅根据用户明确要求改变{目标角度/景别/时段/天候中的必要项}；未提及的维度保持参考图，不主动新增变化。前景{元素}、中景{元素}、后景{元素}维持合理空间关系；{必要的色调、空气透视、材质表面与植被变化}符合物理逻辑。采用单画面构图，不拼图、不分屏；不出现人物、人影或人体轮廓，不生成文字、水印或logo。
