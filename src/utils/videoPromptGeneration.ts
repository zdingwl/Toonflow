...
async function generateForTrack(input: VideoPromptRequest) {
  ...
  try {
        ...
        if (item.sources === "assets") {
          ...
          const assetsData = await u
            ...
            .select("o_assets.id", "o_assets.assetsId", "o_assets.type", "o_assets.name", "o_assets.describe", "o_assets.prompt as assetPrompt", "o_image.filePath",
              "o_assets.faceReferencePath", "o_assets.fullBodyReferencePath", "o_assets.sideReferencePath", "o_assets.backReferencePath")
            .first();
    ...
    const content = h3PromptMode ? buildH3PromptInput(pictureSourceItems, storyboard, targetDuration,
      images.filter((item: any) => item?._type === "assets" && item._reference !== false && ["audio", "video"].includes(item._fileType || item.type))
...
></storyboardItem>`,
    ...
    // The writer sees the same ordered role crops that Ref2VA receives, not only old image prompts.
