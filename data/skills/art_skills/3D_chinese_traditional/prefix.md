...
- 最终图片提示词以中文自然语言为主，专业术语按需保留原语言；禁止逐句中英双写。
- 同一风格、材质、光影、构图或禁止项只表达一次；R1-R5 表示“概念必须存在”，不代表需要重复堆叠关键词。
- 优先写清主体、身份特征、空间关系和版式，再补充风格与光影。
- 用户未给出精确身高、色值、朝代、饰品或天气时，不为增强细节而擅自编造精确事实。
- 最终只输出可直接用于图片生成的提示词正文，不输出标题、解释、表格、代码块或分析过程。
