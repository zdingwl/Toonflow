...
const prompt = "<Picture 1> and <Picture 2> depict Ava. <Picture 3> depicts the deck.";
...
test("H3 paths strictly resolve planned crops and never fall back to a character sheet", () => {
  assert.equal(h3SlotPath({ ...items[0], _referenceRole: "FULL_BODY_FRONT" }), "/front.png");
  assert.equal(h3SlotPath(items[1]), "/deck.png");
  assert.throws(() => h3SlotPath(items[0]), /缺少人物参考类型/);
...
test("concurrent lazy saves use one exact-prompt plan and leave other prompts and tracks separate", async t => {
  ...
  assert.deepEqual(loaded?.slots.map(slot => [slot.assetId, slot.kind, slot.path]), [[1, "FACE", "/face.png"], [1, "FULL_BODY_FRONT", "/front.png"], [2, undefined, "/deck.png"]]);
...
test("runtime restores saved order independently of upload ordering and accepts runtime field aliases", async t => {
  ...
  assert.deepEqual(runtime.map(item => item.path), ["/face.png", "/front.png", "/deck.png"]);
  assert.ok(runtime.every(item => item.fileType === "image" && item.sourceType === "assets" && item.referenceType === "imageReference"));
  assert.deepEqual(runtime.map(item => item.referenceKind), ["FACE", "FULL_BODY_FRONT", undefined]);
});

test("changed image asset sets, asset types, selected crop paths and missing crops require regeneration", async t => {
  const db = fixture(t), plan = await saveH3ReferencePlan(db, 100, prompt, expandH3AssetSlots(items));
  ...
  assert.throws(() => resolveH3ReferencePlan([{ ...items[0], faceReferencePath: "/new-face.png" }, items[1]], plan), /参考图片已变化.*重新生成/);
  assert.throws(() => resolveH3ReferencePlan([{ ...items[0], fullBodyReferencePath: null }, items[1]], plan), /缺少正面全身参考/);
...
test("same prompt cannot silently rebind its existing reference plan", async t => {
  ...
  await assert.rejects(saveH3ReferencePlan(db, 100, prompt, expandH3AssetSlots([{ ...items[0], faceReferencePath: "/new-face.png" }, items[1]])), /不可覆盖历史计划/);
...
test("translation and manual wording changes copy the plan without re-reading directional prose", async t => {
  const db = fixture(t), before = await saveH3ReferencePlan(db, 100, prompt, expandH3AssetSlots(items));
  const translated = "<Picture 1>、<Picture 2>是艾娃。<Picture 3>是甲板。艾娃侧身背对镜头。";
  ...
  assert.deepEqual(resolveH3ReferencePlan(items, (await loadH3ReferencePlan(db, 100, translated))!).map(item => item.path), ["/face.png", "/front.png", "/deck.png"]);
  await assert.rejects(copyH3ReferencePlan(db, 100, prompt, translated.replace("<Picture 3>", "<Picture 4>")), /参考图编号已变化/);
